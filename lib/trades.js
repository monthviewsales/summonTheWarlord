...
function normalizeSlippage(value) {
  ...
  if (!Number.isFinite(num) || num < 0) {
    throw new Error("Invalid slippage in config. Use a non-negative number.");
  }
  return num;
}

function normalizePriorityFee(value) {
  if (typeof value === "string" && value.toLowerCase() === "auto") {
    return "auto";
  }
  const num = Number(value);
  if (!Number.isFinite(num) || num < 0) {
    throw new Error("Invalid priorityFee in config. Use a non-negative number or \"auto\".");
  }
  return num;
}
...
export async function buyToken(mint, amountSol) {
  ...
  const slippage = normalizeSlippage(cfg.slippage);
  const priorityFeeArg = normalizePriorityFee(cfg.priorityFee);
  ...
  notify(
    `Bought ${tokensReceivedDecimal.toFixed(4)} tokens\nFees: ${totalFees.toFixed(4)} (${feePercent?.toFixed(2) ?? "0.00"}%) | Impact: ${priceImpactDisplay}%`,
    "🟢 Buy Completed"
  );
