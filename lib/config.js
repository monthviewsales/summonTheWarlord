...
const BOOLEAN_KEYS = new Set(["showQuoteDetails", "DEBUG_MODE"]);
const STRING_KEYS = new Set(["rpcUrl", "swapAPIKey", "priorityFeeLevel", "txVersion"]);
...
function evaluateConfigValue(key, value) {
  if (key === "slippage") {
    ...
    if (num === null || num < 0) {
      return { error: "Invalid slippage. Use a non-negative number." };
    }
